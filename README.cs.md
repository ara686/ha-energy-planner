...
| `sensor.energy_planner_managed_<source>_suggested_today` | Energie doporučená dnes pro jeden řízený odběr. EV a TUV používají aktuální vstupy modelu, generické odběry zbývající část historického denního odhadu. Atribut `timeline` obsahuje pouze solární sloty skutečně přidělené danému zdroji. Typované alokace zveřejňují úplnost předpovědi a kompaktní podrobnosti konkrétního zdroje. |
